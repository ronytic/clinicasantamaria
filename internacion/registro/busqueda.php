...
if (!empty($_POST)) {
	...
	$pq=$confirmacioncirugia->mostrarTodo("fechaoperacion LIKE '$fechaoperacion' and coddoctor=$idusuario and codpaciente IN(SELECT codpaciente FROM paciente WHERE nombres LIKE '%$nombres%' and paterno LIKE '%$paterno%' and materno LIKE '%$materno%' and ci LIKE '$ci%')");
    $datos=array();
    $i=0;
    foreach($pq as $p){$i++;
