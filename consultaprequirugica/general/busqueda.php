...
if (!empty($_POST)) {
	...
	$pq=$prequirurgico->mostrarTodo("fechaconsulta LIKE '$fechaconsulta'  and codpaciente IN(SELECT codpaciente FROM paciente WHERE nombres LIKE '%$nombres%' and paterno LIKE '%$paterno%' and materno LIKE '%$materno%' and ci LIKE '$ci%') $estado");
    $datos=array();
    $i=0;
    foreach($pq as $p){$i++;
	...
	listadoTabla($titulo,$datos,1,"","","",array("Ver Reporte"=>"verreporte.php"));
}
?>